"""
Script for starting backend server for latex scripts

Taken from:
https://github.com/OrangeX4/Latex-Sympy-Calculator/tree/main
"""
...
def get_latex():
    try:
        return {"data": latex2latex(request.json["data"]), "error": ""}
...
def get_numerical():
    try:
        return {
            "data": latex(
                simplify(
                    latex2sympy(request.json["data"]).subs(variances).doit().doit()
    ...
    try:
        return {
            "data": latex(
                expand(
                    apart(
                        expand_trig(latex2sympy(request.json["data"]).subs(variances))
                    )
                )
            ),
            "error": "",
...
def complex():
    ...
    return {"success": True, "value": is_real}
